...
unsigned int audio_set_i2s_mclk_tune(int up_down,unsigned ppm_value)
{
    ...
    unsigned int audio_mclk = 0;
    unsigned int ret = 0;
    static unsigned long long mclk_offset_value;
    static unsigned long long temp_n2_sdmin;
    ...
    mclk_offset_value = audio_mclk;
    mclk_offset_value = mclk_offset_value*ppm_value;
    ret = do_div(mclk_offset_value,1000000);
    if(ret > 1000000/2)
    	mclk_offset_value += 1;

    temp_n2_sdmin = 2550*1000;
    temp_n2_sdmin = temp_n2_sdmin*1000; 
    temp_n2_sdmin = temp_n2_sdmin*1000*100;// 2.55G x 100000 for improve accurate 
    if(up_down == 0) {
	    audio_mclk = audio_mclk + mclk_offset_value;

    } else if (up_down == 1) {
	    audio_mclk = audio_mclk - mclk_offset_value;
    }
    printk("mclk_offset_value:%lld,set audio_mclk:%d\n",mclk_offset_value,audio_mclk);
    do_div(temp_n2_sdmin,audio_mclk);
    do_div(temp_n2_sdmin,audio_xd);
    printk("temp_n2_sdmin:%lld,audio_xd:%d\n",temp_n2_sdmin,audio_xd);
    
    n2_integer = (unsigned int)temp_n2_sdmin;
    n2_integer = n2_integer/100000;
    
    sdmin_value = (unsigned int)temp_n2_sdmin;
    sdmin_value = (sdmin_value%100000) * 16384;
    ret = do_div(sdmin_value,100000);
    if(ret > 100000/2)
	    sdmin_value += 1;

    mpll_reg_value = (mpll_reg_value & 0xffffc000) | sdmin_value;
    mpll_reg_value = (mpll_reg_value & 0xfe00ffff) | (n2_integer << 16);
    
    printk("%s,audio_xd:%d,pre_mpll_reg:0x%x,sdmin_value:%d,n2_integer:%d,ppm_value:%d,mpll_reg_value:0x%x\n",
